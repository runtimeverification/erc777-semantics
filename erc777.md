...
`#atomicEnd` if the operation completed successfully.
...
Yet there is some extra functionality for the case the receiver hook is not
implemented.
When the receiver address is not a contract address, or the configuration does
not support regular addresses with no receiver hook, the operation reverts.
...
burning for any reason" vs "Tokens MAY be burned from any holder address".
We implement this through the cell `<burnAllowed>`.
If it holds `true`, the burn operation will proceed according to other rules.
If it holds `false`, the burn operation is not allowed for the contract,
so any burn operation will revert.
